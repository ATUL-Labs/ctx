 ...
 *           batch, diff, errors, undo, snapshot, refs, recent, links, guard
 *
 * Response format:
 *   {"ok": true, "output": "...text..."}
...
function processRequest(root, request) {
  ...
  try {
    ...
    // --- errors (browser console errors from lex serve) ---
    if (cmd === 'errors') {
      ...
      for (const port of ports) {
        try {
          ...
          result = JSON.parse(errOut);
          break;
        } catch {}
      }
      if (!result) return { ok: false, error: 'no lex server running - start one with: lex serve or lex watch' };
      const errs = result.errors || [];
      if (!errs.length) return { ok: true, output: 'no console errors captured', count: 0 };
      const lines = [];
    ...
    return { ok: false, error: 'unknown command: ' + cmd + '. Available: search, symbols, grep, read, patch, insert, rename, delete, batch, diff, errors, undo, snapshot, refs, recent, links, guard' };
