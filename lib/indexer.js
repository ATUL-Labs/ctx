  ...
  const rec = (dir, relDir) => {
    ...
    for (const d of entries) {
      ...
      if (d.isDirectory()) {
        ...
        if (relDir === '.lex') { if (d.name === 'pages') rec(path.join(dir, d.name), rel); continue; }
...
function ftsRows(db, terms, limit, mark, root, scope) {
  ...
  const prefixQ = cleanTerms.map(t => t + '*');
  ...
  if (!rows.length) rows = runQ(prefixQ.join(' '));
  if (!rows.length && cleanTerms.length > 1) rows = runQ(prefixQ.join(' OR '));
  // 4. Fuzzy: try progressively shorter prefixes for typo tolerance
  if (!rows.length) {
    for (let plen = Math.max(2, cleanTerms[0].length - 2); plen >= 2 && !rows.length; plen--) {
      const fuzzy = cleanTerms.map(t => t.substring(0, Math.min(plen, t.length)) + '*');
...
function updateFile(db, root, relPath) {
  ...
  if (inCtx && (!rel.endsWith('.md') || rel.startsWith('.lex/sessions/'))) return false;
...
module.exports = { openDb, openDbAt, refresh, refreshDocs, updateFile, ftsRows, walk, loadIgnorePrefixes, loadAgentConfig, shouldRefresh, markRefreshed };
