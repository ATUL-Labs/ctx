...
| Track a todo list | Not available - use `.ctx/wip.md` as the checklist instead |

## Rules-file delivery

Trae reads project rules from `.trae/rules/project_rules.md`. Copy the ctx
...
- No hooks on Trae: continuity is protocol-driven, not automated. Follow the
  step-cadence in `.ctx/wip.md` and update it after every significant step.
- No subagents: all work happens inline in the single Builder session.
- No native todo tool: `.ctx/wip.md` is the checklist of record.
- Trae supports MCP servers through its MCP settings. The codegraph MCP can be
  added there once configured.
- `.ctx/` files are plain markdown, readable and editable with Trae's standard
  file tools.
